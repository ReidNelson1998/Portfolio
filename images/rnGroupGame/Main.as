	...
	public class Main extends Sprite {
		...
		private function update(e: Event): void {
			...
			for each(var meph: Mephistopheles in _meph) {
				...
				if (meph.shadow.hitTestObject(_p.shadow)) {
					if (_playerhp.width <= 5) {
						_playerhp.width = 0;
					} else {
						_playerhp.width -= 5;
			...
			for each(var beel: Beelzebub in _beel) {
				...
				if (beel.shadow.hitTestObject(_p.shadow)) {
					if (_playerhp.width <= 7) {
						_playerhp.width = 0;
					} else {
						_playerhp.width -= 7;
					}
			...
			for each(var bugC: Cloud in _Clouds) {
				...
				if (bugC.shadow.hitTestObject(_p.shadow)) {
					if (_playerhp.width <= 1) {
						_playerhp.width = 0;
					} else {
						_playerhp.width -= 1;
			...
			for each(var satan: Satan in _satan) {
				...
				if (satan.shadow.hitTestObject(_p.shadow)) {
					if (_playerhp.width <= 5) {
						_playerhp.width = 0;
					} else {
						_playerhp.width -= 5;
